...
void Chromosome::EndAddingGenes()
{
    ...
    {
        genes.push_back( random(minvalues[i], maxvalues[i]));
    }
    Colour[0] = random(0,255)/255.f;
    Colour[1] = random(0,255)/255.f;
    Colour[2] = random(0,255)/255.f;
}
...
{
    int point = random(0, dnaLength-1);
    for (int i = 0; i < dnaLength; i++)
    {
        if (i < point)
        {
...
void Chromosome::Mutate(int MutationRate)
{
    ...
    {
        chancethisgeneswaps = random(0, 100);
        if(chancethisgeneswaps > MutationRate)
        {
            if(maxvalues[i] - minvalues[i] > 1)
            {
                bool higher = (bool)random(0, 1);
                if(higher)
                    genes[i] = random(genes[i], maxvalues[i]);
                else
                    genes[i] = random(minvalues[i], genes[i]);
            }
